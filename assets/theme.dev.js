...
;(function (
  ...
  function setVars() {
    const { windowHeight, headerHeight, logoHeight, footerHeight, collectionNavHeight } =
      readHeights()
    ...
    if (document.querySelector("[data-tracking-consent].popup-cookies--bottom")) {
      document.documentElement.style.setProperty(
        "--cookie-bar-height",
        `${document.querySelector("[data-tracking-consent].popup-cookies--bottom").offsetHeight}px`
      )
    }
  }
  ...
  */
  function removeLoadingClassFromLoadedImages(container) {
    container.querySelectorAll(selectors$Y.img).forEach((img) => {
      if (img.complete) {
  ...
  function accessibleLinks(elements, options) {
    ...
    var messages = {
      ...
      newWindowExternal:
        options.messages.newWindowExternal || "Opens external website in a new window.",
    }
  ...
  class HeaderSearchForm extends HTMLElement {
    constructor() {
      ...
      this.input = this.querySelector(selectors$X.inputSearch)
      this.resetButton = this.querySelector(selectors$X.resetButton)
    ...
    shouldResetForm() {
      return !document.querySelector(selectors$X.focusedElements)
    }
    ...
    onFormReset(event) {
      ...
      if (this.shouldResetForm()) {
        this.input.value = ""
        this.toggleResetButton()
        event.target.querySelector(selectors$X.inputSearch).focus()
      }
    }
  }
  ...
  class PredictiveSearch extends HeaderSearchForm {
    constructor() {
      ...
      this.allPredictiveSearchInstances = document.querySelectorAll(selectors$W.predictiveSearch)
      this.cachedResults = {}
      this.input = this.querySelector(selectors$W.searchInput)
      this.isOpen = false
      this.predictiveSearchResults = this.querySelector(selectors$W.predictiveSearchResults)
      this.searchPopdown = this.closest(selectors$W.searchPopdown)
      this.popularSearches = this.searchPopdown?.querySelector(selectors$W.popularSearches)
      this.searchTerm = ""
    }
    ...
    onChange() {
      ...
      if (!this.searchTerm || !newSearchTerm.startsWith(this.searchTerm)) {
        ...
        this.querySelector(selectors$W.searchResultsGroupsWrapper)?.remove()
      }
    ...
    onFormSubmit(event) {
      if (!this.getQuery().length || this.querySelector(selectors$W.selectedLink))
        event.preventDefault()
    }
    ...
    shouldResetForm() {
      return !document.querySelector(selectors$W.selectedLink)
    }
    ...
    updateSearchForTerm(previousTerm, newTerm) {
      const searchForTextElement = this.querySelector(selectors$W.searchForText)
    ...
    switchOption(direction) {
      ...
      const selectedElement = this.querySelector(selectors$W.ariaSelected)

      // Filter out hidden elements (duplicated page and article resources) thanks
      // to this https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/offsetParent
      const allVisibleElements = Array.from(
        this.querySelectorAll(selectors$W.allVisibleElements)
    ...
    selectOption() {
      const selectedOption = this.querySelector(selectors$W.selectedOption)
      ...
      fetch(
        ...
        .then((text) => {
          const resultsMarkup = new DOMParser()
            ...
            .querySelector(selectors$W.sectionPredictiveSearch).innerHTML
    ...
    setLiveRegionLoadingState() {
      this.statusElement =
        this.statusElement || this.querySelector(selectors$W.predictiveSearchStatus)
    ...
    setLiveRegionResults() {
      ...
      this.setLiveRegionText(this.querySelector(selectors$W.searchResultsLiveRegion).textContent)
    }
    ...
    closeResults(clearSearchTerm = false) {
      ...
      const selected = this.querySelector(selectors$W.ariaSelected)
  ...
  class Popout extends HTMLElement {
    ...
    connectedCallback() {
      this.popoutList = this.querySelector(selectors$V.popoutList)
      this.popoutToggle = this.querySelector(selectors$V.popoutToggle)
      this.popoutToggleText = this.querySelector(selectors$V.popoutToggleText)
      this.popoutInput = this.querySelector(selectors$V.popoutInput)
      this.popoutOptions = this.querySelectorAll(selectors$V.popoutOptions)
      this.productGrid = this.popoutList.closest(selectors$V.productGrid)
      this.fireSubmitEvent = this.hasAttribute(attributes$u.submit)
    ...
    onPopupToggleClick(evt) {
      ...
      const ariaExpanded = button.getAttribute(attributes$u.ariaExpanded) === "true"

      if (this.productGrid) {
        const productGridItemImage = this.productGrid.querySelector(selectors$V.productGridImage)
    ...
    toggleListPosition() {
      const button = this.querySelector(selectors$V.popoutToggle)
      const ariaExpanded = button.getAttribute(attributes$u.ariaExpanded) === "true"
    ...
    popupOptionsClick(evt) {
      const link = evt.target.closest(selectors$V.popoutOptions)
      if (link.attributes.href.value === "#") {
        ...
        if (this.fireSubmitEvent) {
          ...
          const targetAttribute = this.popoutList.querySelector(`[${attributes$u.ariaCurrent}]`)
          ...
          if (targetAttribute && targetAttribute.hasAttribute(`${attributes$u.ariaCurrent}`)) {
          ...
          if (attrValue !== "") {
            ...
            if (
              this.popoutToggleText.hasAttribute(attributes$u.popoutToggleText) &&
              this.popoutToggleText.getAttribute(attributes$u.popoutToggleText) !== ""
            ) {
              this.popoutToggleText.setAttribute(attributes$u.popoutToggleText, attrValue)
            }
          }
  ...
  class QuantityCounter extends HTMLElement {
    ...
    onButtonClick(event) {
      ...
      const button =
        event.target.nodeName == "BUTTON" ? event.target : event.target.closest("button")
  ...
  */
  function elementsIntersectionObserver() {
    const elementsToAnimate = document.querySelectorAll(selectors$U.aos)
  ...
  */
  ...
  class DeferredMedia extends HTMLElement {
    constructor() {
      super()
      const poster = this.querySelector(selectors$T.deferredMediaButton)
    ...
    loadContent(focus = true) {
      ...
      if (!this.getAttribute("loaded")) {
        ...
        const mediaElement = this.appendChild(content.querySelector(selectors$T.media))
    ...
    pauseAllMedia() {
      document.querySelectorAll(selectors$T.youtube).forEach((video) => {
        video.contentWindow.postMessage(
          '{"event":"command","func":"' + "pauseVideo" + '","args":""}',
          "*"
        )
      })
      document.querySelectorAll(selectors$T.vimeo).forEach((video) => {
      ...
      if (
        ...
        for (const property in Shopify.recentlyViewed) {
          ...
          if (
            ...
            (objCounter === Object.keys(Shopify.recentlyViewed).length && !addClassesArr.length)
          ) {
        ...
        fetch(
          ...
          .then((product) => {
            ...
            let productReplaced = product.includes("||itemIndex||")
              ...
              : product
            productReplaced = productReplaced.includes("||itemAosDelay||")
              ? productReplaced.replaceAll("||itemAosDelay||", aosDelay)
              : productReplaced
            productReplaced = productReplaced.includes("||itemAosImageDuration||")
              ? productReplaced.replaceAll("||itemAosImageDuration||", aosImageDuration)
              : productReplaced
            productReplaced = productReplaced.includes("||itemAosTextDuration||")
              ? productReplaced.replaceAll("||itemAosTextDuration||", aosTextDuration)
              : productReplaced
            productReplaced = productReplaced.includes("||itemAnimationAnchor||")
              ? productReplaced.replaceAll("||itemAnimationAnchor||", anchorAnimation)
              : productReplaced
  ...
  class ShippingCalculator extends HTMLElement {
    constructor() {
      ...
      this.getRatesButton = this.querySelector(selectors$S.getRates)
      this.fieldsContainer = this.querySelector(selectors$S.addressContainer)
      this.selectCountry = this.querySelector(selectors$S.addressCountry)
      this.selectProvince = this.querySelector(selectors$S.addressProvince)
      this.template = this.querySelector(selectors$S.template)
      this.wrapper = this.querySelector(selectors$S.wrapper)
    ...
    connectedCallback() {
      ...
      if (
        this.selectCountry &&
        this.selectCountry.hasAttribute(attributes$t.default) &&
        this.selectProvince &&
        this.selectProvince.hasAttribute(attributes$t.default)
      ) {
      ...
      if (this.getRatesButton) {
        ...
        if (
          ...
          this.getRatesButton.classList.contains(classes$D.getRatesTrigger)
        ) {
          const zipElem = document.querySelector(selectors$S.addressZip)
    ...
    onButtonClick() {
      ...
      shippingAddress.zip = document.querySelector(selectors$S.addressZip).value || ""
    ...
    render(response) {
      if (this.template && this.wrapper) {
        ...
        if (response.success) {
          ...
          const noShippingElem = createdNewElem.querySelector(selectors$S.templateNoShipping)
  ...
  class CartDrawer {
    ...
    init() {
      ...
      this.cartDrawerClose = document.querySelector(selectors$R.cartDrawerClose)
      ...
      this.buttonHolder = document.querySelector(selectors$R.buttonHolder)
      ...
      this.freeShippingLimit = this.freeShipping.length
        ? Number(this.freeShipping[0].getAttribute(attributes$s.freeShippingLimit)) *
          100 *
    ...
    updateCart(updateData = {}, currentItem = null) {
      ...
      const newItem =
        this.cart.querySelector(`[${attributes$s.item}="${updateData.id}"]`) || currentItem
      const lineIndex = newItem?.hasAttribute(attributes$s.itemIndex)
        ? parseInt(newItem.getAttribute(attributes$s.itemIndex))
        : 0
      const itemTitle = newItem?.hasAttribute(attributes$s.itemTitle)
    ...
    addToCartError(data, button) {
      ...
      if (button !== null) {
        const outerContainer =
          button.closest(selectors$R.outerSection) ||
          button.closest(selectors$R.quickAddHolder) ||
          button.closest(selectors$R.quickAddModal)
        ...
        if (
          buttonUpsellHolder &&
          buttonUpsellHolder.querySelector(selectors$R.formErrorsContainer)
        ) {
      ...
      setTimeout(() => {
        buttons.forEach((button) => {
          ...
          const isVariantUnavailable =
            button.closest(selectors$R.formWrapper)?.classList.contains(classes$C.variantSoldOut) ||
            button
              .closest(selectors$R.formWrapper)
              ?.classList.contains(classes$C.variantUnavailable)
    ...
    closeCartDrawer() {
      ...
      this.cartErrorHolder.classList.remove(classes$C.expanded)
      this.cartDrawer.querySelectorAll(selectors$R.animation).forEach((item) => {
    ...
    build(data) {
      ...
      this.newTotalItems =
        cartItemsData && cartItemsData.querySelectorAll(selectors$R.item).length
          ? cartItemsData.querySelectorAll(selectors$R.item).length
          : 0
      this.subtotal =
        cartTotal && cartTotal.hasAttribute(attributes$s.cartTotal)
          ? parseInt(cartTotal.getAttribute(attributes$s.cartTotal))
          : 0
      this.cartCount = this.getCartItemCount()
    ...
    getCartItemCount() {
      return Array.from(this.cart.querySelectorAll(selectors$R.qtyInput)).reduce(
    ...
    freeShippingMessageHandle(total) {
      ...
      this.freeShipping.forEach((message) => {
        const hasQualifiedShippingMessage =
          message.hasAttribute(attributes$s.freeShipping) &&
          message.getAttribute(attributes$s.freeShipping) === "true" &&
          total >= 0
    ...
    skipUpsellProductEvent() {
      ...
      const skipButtons = this.upsellProductsHolder.querySelectorAll(
        selectors$R.buttonSkipUpsellProduct
      )
        ...
        skipButtons.forEach((button) => {
          button.addEventListener("click", (event) => {
            ...
            const productID = button
              .closest(selectors$R.quickAddHolder)
              .getAttribute(attributes$s.quickAddHolder)
    ...
    removeUpsellProduct(productID) {
      ...
      const upsellProduct = this.upsellProductsHolder.querySelector(
        `[${attributes$s.quickAddHolder}="${productID}"]`
      )
    ...
    observeAdditionalCheckoutButtons() {
      ...
      const additionalCheckoutButtons = this.cartDrawer.querySelector(
        selectors$R.additionalCheckoutButtons
      )
  ...
  class Accessibility {
    ...
    init() {
      ...
      this.inPageLink = document.querySelector(selectors$Q.inPageLink)
      this.linkesWithOnlyHash = document.querySelectorAll(selectors$Q.linkesWithOnlyHash)
  ...
  class MainSearch extends HeaderSearchForm {
    constructor() {
      ...
      this.allSearchInputs = document.querySelectorAll(selectors$P.inputSearch)
  ...
  class SearchPopdown extends HTMLElement {
    constructor() {
      super()
      this.popdown = this.querySelector(selectors$O.popdown)
      this.popdownContainer = this.querySelector(selectors$O.details)
    ...
    connectedCallback() {
      ...
      this.popdown.addEventListener("transitionend", (event) => {
        if (
          ...
          this.popdownContainer.getAttribute("open") == "false"
        ) {
    ...
    onPopdownToggleClick(event) {
      event.preventDefault()
      event.target.closest(selectors$O.details).hasAttribute("open")
        ? this.close()
        : this.open(event)
    }

    onBodyClick(event) {
      if (!this.contains(event.target) || event.target.hasAttribute(attributes$r.popdownUnderlay))
        this.close()
    }

    open(event) {
      this.onBodyClickEvent = this.onBodyClickEvent || this.onBodyClick.bind(this)
      event.target.closest(selectors$O.details).setAttribute("open", "")
      ...
      requestAnimationFrame(() => {
        event.target.closest(selectors$O.details).setAttribute("open", "true")
        this.a11y.trapFocus(this.popdown, {
          elementToFocus: this.popdown.querySelector(selectors$O.input),
        })
      })
    }
  ...
  class CollapsibleElements extends HTMLElement {
    constructor() {
      ...
      this.collapsibles = this.querySelectorAll(selectors$N.collapsible)
      this.single = this.hasAttribute(attributes$q.single)
    }

    connectedCallback() {
      this.collapsibles.forEach((collapsible) => {
        const trigger = collapsible.querySelector(selectors$N.trigger)
        const body = collapsible.querySelector(selectors$N.body)
    ...
    open(collapsible) {
      ...
      const body = collapsible.querySelector(selectors$N.body)
      const content = collapsible.querySelector(selectors$N.content)
    ...
    setBodyHeight(body, contentHeight) {
      body.style.height =
        contentHeight !== "auto" && contentHeight !== "" ? `${contentHeight}px` : contentHeight
    }

    onCollapsibleClick(event) {
      event.preventDefault()

      const trigger = event.target.matches(selectors$N.trigger)
        ? event.target
        : event.target.closest(selectors$N.trigger)
      const collapsible = trigger.closest(selectors$N.collapsible)
  ...
  class Addresses {
    constructor(section) {
      ...
      this.addressNewForm = this.section.querySelector(selectors$M.addressNewForm)
    ...
    init() {
      if (this.addressNewForm) {
        ...
        const newButtons = section.querySelectorAll(selectors$M.btnNew)
        ...
        const editButtons = section.querySelectorAll(selectors$M.btnEdit)
        if (editButtons.length) {
          editButtons.forEach((element) => {
            element.addEventListener("click", function () {
              const formId = this.getAttribute(selectors$M.dataFormId)
              section
                .querySelector(`${selectors$M.editAddress}_${formId}`)
                .classList.toggle(classes$A.hidden)
            })
          })
        }

        const deleteButtons = section.querySelectorAll(selectors$M.btnDelete)
        if (deleteButtons.length) {
          deleteButtons.forEach((element) => {
            element.addEventListener("click", function () {
              const formId = this.getAttribute(selectors$M.dataFormId)
              const confirmMessage = this.getAttribute(selectors$M.dataConfirmMessage)
              if (confirm(confirmMessage)) {
                Shopify.postLink(window.theme.routes.addresses_url + "/" + formId, {
    ...
    customerAddresses() {
      ...
      if (Shopify.CountryProvinceSelector) {
        new Shopify.CountryProvinceSelector(
          selectors$M.addressCountryNew,
          selectors$M.addressProvinceNew,
          {
      ...
      const countryOptions = this.section.querySelectorAll(selectors$M.addressCountryOption)
      countryOptions.forEach((element) => {
        const formId = element.getAttribute(selectors$M.dataFormId)
        const countrySelector = `${selectors$M.addressCountry}_${formId}`
        const provinceSelector = `${selectors$M.addressProvince}_${formId}`
        const containerSelector = `${selectors$M.addressProvinceContainer}_${formId}`
  ...
  const template$1 = document.querySelector(selectors$M.templateAddresses)
  if (template$1) {
  ...
  class Account {
    ...
    init() {
      if (this.section.querySelector(selectors$L.account)) {
        this.accountMobileSidebar()
      }
    }

    accountMobileSidebar() {
      if (this.section.querySelector(selectors$L.accountSidebarMobile)) {
        this.section
          .querySelector(selectors$L.accountSidebarMobile)
  ...
  const template = document.querySelector(selectors$L.accountTemplateLogged)
  if (template) {
  ...
  class Login {
    constructor(form) {
      this.form = form
      this.showButton = form.querySelector(selectors$K.showReset)
      this.hideButton = form.querySelector(selectors$K.hideReset)
      this.recover = form.querySelector(selectors$K.recover)
      this.recoverSuccess = form.querySelector(selectors$K.recoverSuccess)
      this.login = form.querySelector(selectors$K.login)
      this.init()
    }

    init() {
      if (window.location.hash == selectors$K.recoverHash || this.recoverSuccess) {
    ...
    showRecoverPasswordForm() {
      this.login.classList.add(selectors$K.hideClass)
      this.recover.classList.remove(selectors$K.hideClass)
      window.location.hash = selectors$K.recoverHash
      return false
    }

    hideRecoverPasswordForm() {
      this.recover.classList.add(selectors$K.hideClass)
      this.login.classList.remove(selectors$K.hideClass)
      window.location.hash = ""
      return false
    }
  }

  const loginForm = document.querySelector(selectors$K.form)
  if (loginForm) {
    new Login(loginForm)
  }
  ...
  class Section {
    constructor(container, registration) {
      ...
      this.id = container.getAttribute(selectors$J.id)
  ...
  function validateContainerElement(container) {
    ...
    if (container.getAttribute(selectors$J.id) === null) {
      throw new Error(
  ...
  function load(types, containers) {
    ...
    if (typeof containers === "undefined") {
      containers = document.querySelectorAll("[" + selectors$J.type + "]")
    }
    ...
    types.forEach(function (type) {
      ...
      containers = containers.filter(function (container) {
        ...
        if (container.getAttribute(selectors$J.type) === null) {
          return false
        }

        // Keep in list of containers because current type doesn't match
        if (container.getAttribute(selectors$J.type) !== type) {
          return true
        }
  ...
  if (window.Shopify.designMode) {
    document.addEventListener("shopify:section:load", function (event) {
      ...
      var container = event.target.querySelector("[" + selectors$J.id + '="' + id + '"]')

      if (container !== null) {
        load(container.getAttribute(selectors$J.type), container)
      }
    })
  ...
  class Tooltip {
    constructor(el, options = {}) {
      this.tooltip = el
      if (!this.tooltip.hasAttribute(selectors$I.tooltip)) return
      this.label = this.tooltip.getAttribute(selectors$I.tooltip)
    ...
    addPin(stopMouseEnter = false) {
      ...
      if (
        tooltipTarget &&
        ((stopMouseEnter && !this.tooltip.hasAttribute(selectors$I.tooltipStopMouseEnter)) ||
          !stopMouseEnter)
      ) {
  ...
  const tooltipSection = {
    onLoad() {
      sections$n[this.id] = []
      const els = this.container.querySelectorAll(`[${selectors$I.tooltip}]`)
      els.forEach((el) => {
  ...
  class Slider {
    constructor(container, slideshow = null) {
      ...
      this.slideshow = slideshow || this.container.querySelector(selectors$H.slider)

      if (!this.slideshow) return

      this.slideshowSlides = this.slideshow.querySelectorAll(selectors$H.slide)

      if (this.slideshowSlides.length <= 1) return

      this.sliderThumbs = this.container.querySelectorAll(selectors$H.sliderThumb)
      ...
      if (this.slideshow.hasAttribute(attributes$p.sliderOptions)) {
        this.customOptions = JSON.parse(
          decodeURIComponent(this.slideshow.getAttribute(attributes$p.sliderOptions))
        )
      }
      ...
      this.sliderOptions = {
        ...
        on: {
          ...
          change: (index) => {
            ...
            const elementsToAnimate = slide.querySelectorAll(selectors$H.aos)
            if (elementsToAnimate.length) {
    ...
    slideActions(changeEvent = false) {
      ...
      const currentSlideLink = currentSlide.querySelector(selectors$H.link)
      const buttons = this.slideshow.querySelectorAll(
        `${selectors$H.slide} a, ${selectors$H.slide} button`
      )

      if (
        document.body.classList.contains(classes$y.focused) &&
        currentSlideLink &&
      ...
      if (buttons.length) {
        buttons.forEach((button) => {
          const slide = button.closest(selectors$H.slide)
          if (slide) {
      ...
      if (
        ...
        currentSlide.hasAttribute(attributes$p.slideIndex)
      ) {
        const slideIndex = parseInt(currentSlide.getAttribute(attributes$p.slideIndex))
        const currentThumb = this.container.querySelector(
          `${selectors$H.sliderThumb}.${classes$y.isSelected}`
        )
        if (currentThumb) {
    ...
    positionArrows() {
      if (
        this.slideshow.hasAttribute(attributes$p.arrowPositionMiddle) &&
        this.sliderOptions.prevNextButtons
      ) {
        const itemImage =
          this.slideshow.querySelector(selectors$H.collectionImage) ||
          this.slideshow.querySelector(selectors$H.productItemImage) ||
          this.slideshow.querySelector(selectors$H.columnImage)

        // Prevent 'clientHeight' of null error if no image
        if (!itemImage) return

        this.slideshow.querySelector(selectors$H.flickityPrevArrow).style.top =
          itemImage.clientHeight / 2 + "px"
        this.slideshow.querySelector(selectors$H.flickityNextArrow).style.top =
          itemImage.clientHeight / 2 + "px"
      }
    }
    ...
    equalizeHeight() {
      Flickity.prototype._createResizeClass = function () {
        requestAnimationFrame(() => {
          this.element.classList.add(classes$y.flickityResize)
        })
      }
      this.flkty._createResizeClass()
      const resize = Flickity.prototype.resize
      Flickity.prototype.resize = function () {
        this.element.classList.remove(classes$y.flickityResize)
        this.element.classList.add(classes$y.flickityResizing)
        resize.call(this)
        requestAnimationFrame(() => {
          this.element.classList.remove(classes$y.flickityResizing)
          this.element.classList.add(classes$y.flickityResize)
        })
      }
    }
    ...
    onBlockSelect(evt) {
      ...
      const slide = this.slideshow.querySelector(
        `[${selectors$H.slideValue}="${evt.detail.blockId}"]`
      )
  ...
  const slider = {
    onLoad() {
      sections$l[this.id] = []
      const els = this.container.querySelectorAll(selectors$H.slider)
      els.forEach((el) => {
  ...
  const scrollTo = (elementTop) => {
    /* Sticky header check */
    const headerHeight =
      document.querySelector(selectors$G.headerSticky) &&
      document.querySelector(selectors$G.headerHeight)
        ? document.querySelector(selectors$G.headerHeight).getBoundingClientRect().height
        : 0
  ...
  class NewsletterCheckForResult {
    constructor(newsletter) {
      ...
      if (this.popup) {
        this.cookie = new PopupCookie(
          this.popup.getAttribute(attributes$o.cookieNameAttribute),
          "user_has_closed",
          null
        )
      }
    ...
    showMessage() {
      ...
      if (this.newsletter.id === this.formID) {
        ...
        const newsletterHeading = newsletter.parentElement.querySelector(
          selectors$F.newsletterHeading
        )
  ...
  class CollectionFiltersForm extends HTMLElement {
    constructor() {
      ...
      this.collectionSidebarSlideOut = this.container.querySelector(
        selectors$E.collectionSidebarSlideOut
      )
    ...
    connectedCallback() {
      ...
      if (this.sort) {
        document.addEventListener("theme:filter:sort-update", this.submitFormEvent)
      }
    ...
    showMore(e) {
      ...
      e.target.parentElement.previousElementSibling
        .querySelectorAll(selectors$E.linkHidden)
        .forEach((link, index) => {
    ...
    updatePrice(e) {
      ...
      if (
        type === selectors$E.input ||
        type === selectors$E.select ||
        type === selectors$E.label ||
    ...
    filterUpdateFromUrl(e) {
      const target = e.target
      if (
        target.matches(selectors$E.filterUpdateUrlButton) ||
        (target.closest(selectors$E.filterUpdateUrlButton) && target)
      ) {
        e.preventDefault()
        const button = target.matches(selectors$E.filterUpdateUrlButton)
          ? target
          : target.closest(selectors$E.filterUpdateUrlButton)
    ...
    submitForm(e, replaceHref = "") {
      console.log("ay")
      this.sort = document.querySelector(`[${selectors$E.sort}]`)
      ...
      if (!e || (e && e.type !== "popstate")) {
        if (replaceHref === "") {
          ...
          if (
            filterUrlRemoveString.includes("filter.") ||
            filterUrlRemoveString.includes("page=")
          ) {
          ...
          if (this.form) {
            ...
            const rangeMinDefaultValue =
              rangeMin && rangeMin.hasAttribute(selectors$E.rangeMinDefault)
                ? rangeMin.getAttribute(selectors$E.rangeMinDefault)
                : ""
            const rangeMaxDefaultValue =
              rangeMax && rangeMax.hasAttribute(selectors$E.rangeMaxDefault)
            ...
            for (let [key, val] of formParams.entries()) {
              if (key.includes("filter.") && val) {
                ...
                if (
                  (val === rangeMinDefaultValue && key === "filter.v.price.gte") ||
                  (val === rangeMaxDefaultValue && key === "filter.v.price.lte")
                ) {
        ...
        fetch(`${window.location.pathname}${window.location.search}`)
          ...
          .then((data) => {
            this.productsContainer.innerHTML = new DOMParser()
              ...
              .querySelector(selectors$E.productsContainer).innerHTML

            if (this.sidebar) {
              this.sidebar.innerHTML = new DOMParser()
                .parseFromString(data, "text/html")
                .querySelector(selectors$E.collectionSidebar).innerHTML

              const activeFiltersCountContainer = this.sidebar.querySelector(
                `[${selectors$E.activeFiltersCount}]`
              )
              const activeFiltersContainer = this.container.querySelectorAll(
                selectors$E.activeFilters
              )
              if (activeFiltersCountContainer && activeFiltersContainer.length) {
                const activeFiltersCount = parseInt(
                  activeFiltersCountContainer.getAttribute(selectors$E.activeFiltersCount)
                )
            ...
            if (this.collectionNav) {
              scrollTo(
                this.productsContainer.getBoundingClientRect().top - this.collectionNav.offsetHeight
              )
            }
    ...
    updateRange(e) {
      if (this.form && typeof this.form.submit === "function") {
        ...
        if (
          checkElements &&
          rangeMin.hasAttribute(selectors$E.rangeMinValue) &&
          rangeMax.hasAttribute(selectors$E.rangeMaxValue)
        ) {
    ...
    disconnectedCallback() {
      ...
      if (this.sort) {
        document.removeEventListener("theme:filter:sort-update", this.submitFormEvent)
      }
  ...
  class RangeSlider extends HTMLElement {
    ...
    setMinValue(minValue) {
      ...
      this.touchLeft.style.left =
        Math.ceil(ratio * (this.offsetWidth - (this.touchLeft.offsetWidth + this.normalizeFact))) +
        "px"
    ...
    setMaxValue(maxValue) {
      ...
      this.touchRight.style.left =
        Math.ceil(
          ratio * (this.offsetWidth - (this.touchLeft.offsetWidth + this.normalizeFact)) +
            this.normalizeFact
        ) + "px"
      this.lineSpan.style.marginLeft = this.touchLeft.offsetLeft + "px"
      this.lineSpan.style.width = this.touchRight.offsetLeft - this.touchLeft.offsetLeft + "px"
    ...
    onMove(event) {
      ...
      this.onChange(
        this.getAttribute(selectors$D.dataMinValue),
        this.getAttribute(selectors$D.dataMaxValue)
      )
    }
  ...
  class Collection {
    constructor(section) {
      ...
      this.sort = document.querySelector(`[${selectors$C.dataSort}]`)
      this.sortButton = document.querySelector(selectors$C.sortButton)
      this.sortLinks = document.querySelectorAll(".popout-list__option[data-sort-link]")
      this.collectionSidebar = this.container.querySelector(selectors$C.collectionSidebar)
      this.collectionSidebarCloseButtons = this.container.querySelectorAll(
        selectors$C.collectionSidebarCloseButton
      )
    ...
    init() {
      ...
      if (this.collectionSidebar) {
        ...
        this.collectionSidebar.addEventListener("theme:filter:update", () => {
          const sidebarCloseButtons = this.container.querySelectorAll(
            selectors$C.collectionSidebarCloseButton
          )
    ...
    sortActions(link, submitForm = true) {
      ...
      const sortActive = this.sort.querySelector(`.${classes$u.active}`)
      if (sortButtonText) {
        const linkText = link ? link.textContent.trim() : ""
        sortButtonText.textContent = linkText
      }
      if (sortActive) {
        sortActive.classList.remove(classes$u.active)
      }
      this.sort.classList.toggle(classes$u.active, link)

      if (link) {
        link.parentElement.classList.add(classes$u.active)
    ...
    initSort() {
      ...
      if (this.sortButton) {
        this.sortButton.addEventListener("click", () => {
          const isFiltersSidebarOpen = this.collectionSidebar.classList.contains(classes$u.expanded)
    ...
    showSidebarCallback() {
      const collectionSidebarSlideOut = this.container.querySelector(
        selectors$C.collectionSidebarSlideOut
      )
  ...
  class Ticker {
    ...
    listen() {
      this.frame.addEventListener("theme:marquee:refresh", this.checkWidthEvent)
      this.checkWidth()
    }

    checkWidth() {
      const padding = window.getComputedStyle(this.frame).paddingLeft.replace("px", "") * 2
      const speed = this.frame.getAttribute(attributes$n.speed)
        ? this.frame.getAttribute(attributes$n.speed)
        : settings$2.textAnimationTime
      ...
      if (this.frame.clientWidth - padding < this.comparitor.clientWidth || this.stopClone) {
        ...
        if (this.scale.childElementCount === 1) {
          ...
          const animationTimeFrame = (
            (this.text.clientWidth / settings$2.space) *
            Number(speed)
          ).toFixed(2)
  ...
  class AnnouncementBar extends HTMLElement {
    constructor() {
      ...
      this.slides = this.querySelectorAll(selectors$A.barSlide)
      this.frames = this.querySelectorAll(selectors$A.frame)
      this.slider = this.querySelector(selectors$A.slider)
      this.marquee = this.querySelector(selectors$A.marquee)
      this.enableSlider =
    ...
    removeAnnouncement() {
      for (let index = 0; index < this.slides.length; index++) {
        ...
        if (
          this.locationPath.indexOf(element.getAttribute(attributes$m.targetReferrer)) === -1 &&
    ...
    initSlider(event) {
      ...
      if (
        (windowWidth >= theme.sizes.small && this.enableSlider) ||
        (windowWidth < theme.sizes.small && !this.enableSlider)
      ) {
        let slidesSelector = ""
        ...
        if (windowWidth >= theme.sizes.small && this.enableSlider) {
          this.enableSlider = false
          slidesSelector = `${selectors$A.barSlide}:not(.${classes$s.mobile})`
        } else if (windowWidth < theme.sizes.small && !this.enableSlider) {
          this.enableSlider = true
          slidesSelector = `${selectors$A.barSlide}:not(.${classes$s.desktop})`
        }

        this.slider = new Slider(this, this.querySelector(selectors$A.slider), slidesSelector)
        this.slider.flkty?.reposition()
      }

      // Init tickers if there's only one slide
    ...
    onTickerResize() {
      this.frames.forEach((element) => {
        element.dispatchEvent(new CustomEvent("theme:marquee:refresh"))
      })
    }

    toggleTicker(e, isStopped) {
      const tickerScale = this.querySelector(selectors$A.tickerScale)
      const element = this.querySelector(`[${attributes$m.slide}="${e.detail.blockId}"]`)
      ...
      if (isStopped && element) {
        ...
        tickerScale.querySelectorAll(selectors$A.tickerText).forEach((textHolder) => {
          textHolder.classList.remove(classes$s.tickerAnimated)
      ...
      if (!isStopped && element) {
        tickerScale.querySelectorAll(selectors$A.tickerText).forEach((textHolder) => {
          textHolder.classList.add(classes$s.tickerAnimated)
  ...
  class HeaderMobileSliderule {
    ...
    trapFocusSliderule(showSliderule = true) {
      ...
      if (trapFocusButton && this.drawerContent) {
        this.accessibility.trapFocus(this.drawerContent, {
          elementToFocus: document.body.classList.contains(classes$o.focused)
            ? trapFocusButton
            : null,
        })
      }
    ...
    hideSliderule(close = false) {
      ...
      const hiddenSelector = close
        ? `[${selectors$v.dataAnimates}].${classes$o.isHidden}`
        : `[${selectors$v.dataAnimates}="${newPosition}"]`
      ...
      const children = close
        ? this.pane.querySelectorAll(`.${classes$o.isVisible}, .${classes$o.isHiding}`)
        : this.children
    ...
    showSliderule() {
      ...
      const hiddenItems = this.pane.querySelectorAll(
        `[${selectors$v.dataAnimates}="${oldPosition}"]`
      )
      if (hiddenItems.length) {
    ...
    closeSliderule() {
      if (
        this.pane &&
        this.pane.hasAttribute(selectors$v.sliderulePane) &&
        parseInt(this.pane.getAttribute(selectors$v.sliderulePane)) > 0
      ) {
  ...
  class Header {
    constructor(el) {
      ...
      const firstSectionOverlayHeader = document.querySelector(
        selectors$u.firstSectionOverlayHeader
      )
      document.body.classList.toggle(
        classes$n.transparent,
        this.wrapper.hasAttribute(attributes$k.transparent)
      )
      ...
      if (this.slider && this.slides.length && this.thumbs.length) {
        ...
        this.thumbs.forEach((thumb, i) => {
          thumb.addEventListener("click", (e) => {
            e.preventDefault()
            const idx =
              thumb.hasAttribute(attributes$j.thumbValue) &&
              thumb.getAttribute(attributes$j.thumbValue) !== ""
                ? parseInt(thumb.getAttribute(attributes$j.thumbValue))
                : i
            ...
            if (isMobile()) {
              ...
              const headerHeight =
                document.querySelector(selectors$s.headerSticky) &&
                document.querySelector(selectors$s.headerHeight)
                  ? document.querySelector(selectors$s.headerHeight).getBoundingClientRect().height
                  : 0
              ...
              if (sliderContainer) {
                const sliderContainerTop =
                  sliderContainer.getBoundingClientRect().top + window.scrollY
  ...
  class ProductFormReader {
    ...
    plan(variant) {
      ...
      const id =
        sellingPlanCheckedValue && sellingPlanCheckedValue !== "" ? sellingPlanCheckedValue : null
    ...
    getPlan(variant) {
      ...
      const id = params.has("selling_plan") ? params.get("selling_plan") : null
  ...
  class NativeScrollbar {
    ...
    goToNext() {
      const moveWith = this.scrollbar.hasAttribute(selectors$q.scrollbarSlideFullWidth)
    ...
    toggleNextArrow() {
      setTimeout(() => {
        this.arrowNext.classList.toggle(
          classes$l.hidden,
          Math.round(
            this.scrollbar.scrollLeft + this.scrollbar.getBoundingClientRect().width + 1
  ...
  class GridSwatch extends HTMLElement {
    ...
    init() {
      ...
      this.swatches.forEach((swatch) => {
        ...
        if (variant) {
          ...
          const swatchHandle = swatch.trim().toLowerCase().replaceAll(" ", "-")
    ...
    showVariantImage(event) {
      ...
      const variantImageSelected = this.productImage.querySelector(
        `[${attributes$i.variantTitle}="${variantName}"]`
      )
    ...
    hideVariantImages() {
      // Hide all variant images
      this.productImage
        .querySelectorAll(`[${attributes$i.variantTitle}].${classes$k.visible}`)
        ?.forEach((image) => {
          image.classList.remove(classes$k.visible)
        })
    }
  ...
  class SiblingSwatches {
    ...
    cacheDefaultValues() {
      this.productLinkValue = this.productLinks[0].hasAttribute(attributes$h.productLink)
    ...
    showSibling(event) {
      ...
      const siblingLink = swatch.hasAttribute(attributes$h.siblingLink)
        ? swatch.getAttribute(attributes$h.siblingLink)
        : ""
      const siblingPrice = swatch.hasAttribute(attributes$h.siblingPrice)
        ? swatch.getAttribute(attributes$h.siblingPrice)
        : ""
      const siblingCutline = swatch.hasAttribute(attributes$h.siblingCutline)
        ? swatch.getAttribute(attributes$h.siblingCutline)
        : ""
      const siblingImage = swatch.hasAttribute(attributes$h.siblingImage)
        ? swatch.getAttribute(attributes$h.siblingImage)
        : ""
  ...
  class GlobalTabs {
        ...
        tabsNavList.forEach((element) => {
          ...
          element.addEventListener("keyup", (event) => {
            if (
              ...
              this.body.classList.contains(classes$i.focused)
            ) {
              this.tabChange(element, tab)
            }
          })
    ...
    tabChange(element, tab) {
      ...
      const currentTabContent = this.container.querySelector(
        `${selectors$n.tabContent}.${classes$i.current}`
      )
    ...
    checkRecentTab() {
      ...
      if (tabLink) {
        ...
        const tabContent = this.container.querySelector(
          `${selectors$n.tabContent}[${selectors$n.dataTabIndex}="${tabLinkIdx}"]`
        )
    ...
    onBlockSelect(evt) {
      const element = this.container.querySelector(
        `${selectors$n.tabLink}[${selectors$n.blockId}="${evt.detail.blockId}"]`
      )
      if (element) {
  ...
  class InitSlider {
    constructor(section, modalHolder = null) {
      ...
      this.mobileSliderDisabled =
        this.container.getAttribute(selectors$m.mobileSliderDisabled) === "true"
      ...
      if (this.slideshow && this.slideshow.hasAttribute(attributes$g.sliderOptions)) {
        this.customOptions = JSON.parse(
          decodeURIComponent(this.slideshow.getAttribute(attributes$g.sliderOptions))
        )
      }
    ...
    createSlider() {
        ...
        on: {
          ready: function () {
            ...
            slides.addEventListener("keyup", (e) => {
              if (e.code === "Enter") {
                const zoomElement = slides.querySelector(
                  `.${classes$h.selected} ${selectors$m.zoomElement}`
                )
                if (zoomElement) {
      ...
      this.flkty.on("change", function (index) {
        ...
        if (instance.thumbSlider) {
          ...
          const currentSlide = instance.thumbSlider.querySelector(
            `${selectors$m.thumb} [${selectors$m.dataThumbIndex}="${index}"]`
          )
      ...
      this.flkty.on("settle", function () {
        ...
        const otherMedia = Array.prototype.filter.call(
        ...
        if (deferredMedia && deferredMedia.getAttribute("loaded") !== true) {
          currentMedia
            .querySelector(selectors$m.deferredMediaButton)
      ...
      if (this.thumbLinks.length) {
        this.thumbLinks.forEach((item) => {
          item.addEventListener("click", (e) => {
            e.preventDefault()
            const slideIdx = item.hasAttribute(selectors$m.dataThumbIndex)
              ? parseInt(item.getAttribute(selectors$m.dataThumbIndex))
              : 0
  ...
  class QuickAddProduct extends HTMLElement {
    ...
    modalCreate(response) {
      const cachedModal = document.querySelector(
        `${selectors$l.quickAddModal}[${attributes$f.productId}="${this.productId}"]`
      )
      ...
      if (cachedModal) {
        ...
        this.modal.querySelector(selectors$l.modalContent).innerHTML = new DOMParser()
          .parseFromString(response, "text/html")
          .querySelector(selectors$l.apiContent).innerHTML
    ...
    modalOpen() {
      ...
      const focusTarget =
        this.modal.querySelector("[autofocus]") || this.modal.querySelector(selectors$l.focusable)
      focusTarget.focus()
        ...
        if (this.hasAttribute(attributes$e.url) && this.getAttribute(attributes$e.url) !== "") {
          fetch(this.getAttribute(attributes$e.url))
            ...
            .then((text) => {
              ...
              if (
                html.querySelector(
                  `${selectors$k.complementaryProducts} ${selectors$k.quickAddProduct}`
                )
              ) {
  ...
  class ProductSticky {
    ...
    stickyScrollCheck() {
      const targetFormWrapper = this.container.querySelector(
        `${selectors$i.productPage} ${selectors$i.formWrapper}`
      )
    ...
    calculateStickyPosition(e = null) {
      ...
      const heightDifference =
        window.innerHeight - formWrapperHeight - this.defaultTopBottomSpacings
      ...
      if (this.stickyFormLoad) {
        if (
          document.querySelector(selectors$i.headerSticky) &&
          document.querySelector(selectors$i.headerHeight)
        ) {
          this.stickyDefaultTop = parseInt(
            document.querySelector(selectors$i.headerHeight).getBoundingClientRect().height
          )
        } else {
  ...
  class ProductForm extends HTMLElement {
    ...
    connectedCallback() {
      ...
      if (this.remainingWrapper) {
        ...
        if (remainingMaxWrap) {
          this.remainingMaxInt = parseInt(
            remainingMaxWrap.getAttribute(attributes$d.remainingMaxAttr),
            10
          )
      ...
      this.enableHistoryState =
        this.container.getAttribute(attributes$d.dataEnableHistoryState) === "true"
      ...
      if (addToCartText.length) {
        setTimeout(() => {
          addToCartText.forEach((element) => {
            if (variant) {
              if (variant.available) {
                if (document.querySelector(".subscription-radio.rc-radio--active") != null) {
                  ...
                  if (variant.compare_at_price > variant.price) {
                    ...
                    let x = +"$" + variant.price
                    let lenx = x.length
                    x = x.substring(0, lenx - 2) + "." + x.substring(lenx - 2)

                    let z = +"$" + variant.price
                    let lenz = z.length
                    z = z.substring(0, lenz - 2) + "." + z.substring(lenz - 2)

                    element.innerHTML =
                      y + "<span class='crossout-price'>" + x + "</span>" + +z + ""
                  } else {
                    let x = addText + " | " + "$" + variant.price
                    var len = x.length
                    x = x.substring(0, len - 2) + "." + x.substring(len - 2)
                    element.innerHTML = x
                  }
                }
              } else {
                let x = theme.strings.soldOut + " | " + "$" + variant.price
                var len = x.length
                x = x.substring(0, len - 2) + "." + x.substring(len - 2)
                element.innerHTML = x
              }
      ...
      if (variant) {
        ...
        if (variant.featured_media) {
          const newImg = this.container.querySelector(
            `[${attributes$d.dataImageId}="${variant.featured_media.id}"]`
          )
          ...
          if (newImageParent) {
            const newImagePos = Array.from(newImageParent.parentElement.children).indexOf(
              newImageParent
            )
  ...
  class ProductModal extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.focusTarget =
        this.modal.querySelector("[autofocus]") || this.modal.querySelector(selectors$e.focusable)
      this.isAnimating = false
    ...
    fetchPickupAvailability(event) {
      ...
      const variantID =
        event && event.detail.variant
          ...
          : this.getAttribute(selectors$d.pickupContainer)

      if (variantID) {
        fetch(
          `${window.theme.routes.root}variants/${variantID}/?section_id=api-pickup-availability`
        )
          .then(this.handleErrors)
          .then((response) => response.text())
          .then((text) => {
            const pickupAvailabilityHTML = new DOMParser()
              .parseFromString(text, "text/html")
              .querySelector(selectors$d.shopifySection).innerHTML
    ...
    openDrawer() {
      if (this.drawer) {
        ...
        const focusTarget =
          ...
          this.drawer.querySelector(selectors$d.focusable)
        focusTarget.focus()
  ...
  class LoadPhotoswipe {
    ...
    initLoader() {
      if (
        this.pswpElement.classList.contains(classes$9.customLoader) &&
        this.options !== "" &&
    ...
    onClose() {
      ...
      if (
        ...
        document.body.classList.contains(classes$9.isFocused)
      ) {
        requestAnimationFrame(() => {
    ...
    setCurrentThumb() {
      const lastCurrentThumb = this.pswpElement.querySelector(
        `${selectors$c.popupThumbs} > .${classes$9.current}`
      )
      if (lastCurrentThumb) {
      ...
      this.zoomWrappers.forEach((elementImage) => {
        ...
        if (this.zoomWrappers.length === counter) {
          const options = {
            ...
            mainClass:
              counter === 1
                ? `${classes$8.popupClass} ${classes$8.popupClassNoThumbs}`
                : `${classes$8.popupClass}`,
  ...
  class Product {
    constructor(section) {
      ...
      if (this.cartBarExist) {
        ...
        root.style.setProperty('--cart-bar-height', `${cartBarHeight}px`);
      }
    }
    ...
    truncateText() {
      ...
      const truncateElementClass = this.truncateElement.getAttribute(
    ...
    cartBarScroll() {
      ...
      const element =
        theme.variables.productPageSticky && this.formWrapper ? this.formWrapper : this.form
      ...
      if (element && this.cartBar) {
        ...
        const georgiasChatElem = document.querySelector("#chat-button")

        this.cartBar.classList.toggle(classes$7.visible, checkPosition)

        if (georgiasChatElem) georgiasChatElem.classList.toggle(classes$7.visible, checkPosition)
      }
    }
    ...
    onBlockSelect(e) {
      ...
      if (slider && this.slider.length) {
        const sliderIndex = slider.hasAttribute(selectors$a.sliderIndex)
  ...
  class Related {
      ...
      fetch(requestUrl)
        ...
        .then((data) => {
          ...
          relatedContent.innerHTML = new DOMParser()
            .parseFromString(data, "text/html")
            .querySelector(selectors$9.apiRelatedProductsTemplate).innerHTML
    ...
    recent() {
      ...
      const howManyToshow = recentlyViewed
        ? parseInt(recentlyViewed.getAttribute(attributes$8.limit))
        : 4
      ...
      Shopify.Products.showRecentlyViewed({
        ...
        onComplete: (wrapper, section) => {
          ...
          const minimumNumberProducts = recentlyViewedHolder.hasAttribute(attributes$8.minimum)
            ? parseInt(recentlyViewedHolder.getAttribute(attributes$8.minimum))
            : 4
          const checkRecentInRelated = !recentlyViewedWrapper && recentProducts.length > 0
          const checkRecentOutsideRelated =
            recentlyViewedWrapper && recentProducts.length >= minimumNumberProducts
  ...
  class LogoList {
    ...
    onBlockSelect(evt) {
      ...
      const slide = this.slideshowNav.querySelector(
        `[${attributes$7.slideData}="${evt.detail.blockId}"]`
      )
  ...
  class CompareImages {
    constructor(section) {
      ...
      if (this.imagesHolder) {
        ...
        this.startPosition = this.imagesHolder.hasAttribute(selectors$6.dataStartPosition)
          ? this.imagesHolder.getAttribute(selectors$6.dataStartPosition)
          : 0
        this.startX = 0
        this.x = 0
  ...
  styles.light = [
    {
      ...
      elementType: "labels",
      stylers: [{ visibility: "simplified" }, { lightness: "64" }, { hue: "#ff0000" }],
    },
    {
      featureType: "administrative",
    ...
    { featureType: "road", elementType: "all", stylers: [{ saturation: "-100" }] },
    {
      featureType: "road",
      elementType: "labels",
      stylers: [{ lightness: "25" }, { gamma: "1.06" }, { saturation: "-100" }],
    },
    ...
    },
    {
      featureType: "road.local",
      elementType: "labels",
      stylers: [{ visibility: "simplified" }, { weight: "0.01" }, { lightness: "39" }],
    },
    {
      featureType: "road.local",
      elementType: "labels.text.stroke",
  ...
  styles.dark_label = [
    ...
    },
    {
      featureType: "administrative",
      elementType: "labels.text.stroke",
      stylers: [{ invert_lightness: true }, { weight: "7.15" }],
    },
    {
      featureType: "landscape",
      elementType: "geometry",
  ...
  class Map {
    ...
    initMaps() {
      ...
      apiLoaded
        .then(() => {
          return this.enableCorrection === "true" && this.lat !== "" && this.long !== ""
  ...
  class TargetReferrer {
    ...
    init() {
      if (
        this.locationPath.indexOf(this.el.getAttribute(selectors$5.dataTargetReferrer)) === -1 &&
        !window.Shopify.designMode
      ) {
    ...
    constructor(el) {
      ...
      window.Shopify.loadFeatures(
        ...
        (error) => {
          ...
          this.showPopup =
            !userCanBeTracked && userTrackingConsent === "no_interaction" && this.enable
  ...
  class ScrollSpy {
    ...
    runObserver(anchorSpy) {
      ...
      const stickyHeader = Boolean(document.querySelector(selectors$2.headerSticky))
      headerHeight = stickyHeader ? headerHeight : 0
      ...
      this.observers[anchorSpy.id] = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          const anchorOld = this.container.querySelector(
            `[${attributes$2.scrollSpy}].${classes$1.selected}`
          )
          const anchorNew = this.container.querySelector(
            `[${attributes$2.scrollSpy}="#${entry.target.id}"]`
          )

          if (
            entry.intersectionRatio > 0.5 &&
            entry.boundingClientRect.top - headerHeight <= entry.boundingClientRect.height
          ) {
  ...
  class CollectionsHover {
    ...
    updateState(targetId) {
      ...
      const buttonSelected = this.container.querySelector(
  ...
  class FeaturedProduct {
    constructor(section) {
      ...
      this.calculateHorizontalPositionEvent = (e) => this.calculateHorizontalPosition(e)
    ...
    init() {
      if (this.horizontalScroll && this.imagesHolder) {
        document.addEventListener("theme:scroll", this.calculateHorizontalPositionEvent)
      }
    ...
    calculateHorizontalPosition(e) {
      let scrollTop = e.detail.position + this.headerHeight
      ...
      this.container.style.setProperty("--translateX", `${percent * -1}%`)
    }
    ...
    getHeaderHeight() {
      return document.querySelector(selectors.headerSticky) &&
        document.querySelector(selectors.headerHeight)
        ? parseInt(document.querySelector(selectors.headerHeight).getBoundingClientRect().height)
        : 0
    }
          ...
          if (mutation.type === "childList") {
            ...
            if (georgiasChatElem) {
              document.addEventListener("theme:scroll", () => {
                scrollTopButton.classList.contains("is-visible")
                  ? georgiasChatElem.classList.add("is-btt-visible")
                  : georgiasChatElem.classList.remove("is-btt-visible")
              })
              observer.disconnect()
              break
            }
